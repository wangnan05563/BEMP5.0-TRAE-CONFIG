...
class DocumentBuilder {
            ...
            sections: [{
                ...
                headers: {
                    default: new Header({
                        children: [new Paragraph({
                            alignment: AlignmentType.CENTER,
                            children: [new TextRun({ text: '项目文档', size: 20, font: FONT.SONG })]
                        })]
                    })
                },
                footers: {
    ...
    async generateUnitTestReportDocument(moduleName, outputPath, templateData) {
        const data = this._resolvePlaceholders(
            templateData || this._getDefaultTemplateData(moduleName, 'unit-test-report'),
            { moduleName, currentDate: new Date().toLocaleDateString('zh-CN') }
        );
        const children = [];
        children.push(...this.createCoverPage(data.coverPage));
    ...
    generateMarkdown(moduleName, outputPath, type = 'design', templateData = null) {
        ...
        const company = (templateData && templateData.coverPage && templateData.coverPage.company) || (this.profile.company && this.profile.company.name) || '公司名称';
