    ...
    paragraphStyles: [
        {
            ...
            run: { size: SIZE.SAN, bold: true, font: FONT.HEI, color: '000000' },
            paragraph: { spacing: { before: 360, after: 240 }, outlineLevel: 0 }
        },
        {
            id: 'Heading2', name: 'Heading 2', basedOn: 'Normal', next: 'Normal', quickFormat: true,
            run: { size: SIZE.SI, bold: true, font: FONT.HEI, color: '000000' },
            paragraph: { spacing: { before: 240, after: 180 }, outlineLevel: 1 }
        },
        {
            id: 'Heading3', name: 'Heading 3', basedOn: 'Normal', next: 'Normal', quickFormat: true,
            run: { size: SIZE.XIAOSI, bold: true, font: FONT.HEI, color: '000000' },
            paragraph: { spacing: { before: 180, after: 120 }, outlineLevel: 2 }
        },
        {
            id: 'Heading4', name: 'Heading 4', basedOn: 'Normal', next: 'Normal', quickFormat: true,
            run: { size: SIZE.WU, bold: true, font: FONT.HEI, color: '000000' },
            paragraph: { spacing: { before: 120, after: 80, indent: { left: 480 } }, outlineLevel: 3 }
        },
        {
            id: 'Heading5', name: 'Heading 5', basedOn: 'Normal', next: 'Normal', quickFormat: true,
            run: { size: SIZE.WU, bold: true, font: FONT.HEI, color: '000000' },
            paragraph: { spacing: { before: 80, after: 60, indent: { left: 960 } }, outlineLevel: 4 }
        }
    ]
};
...
function validatePaths() {
    const warnings = [];
    const criticalDirs = [
        { name: 'templateDir', path: paths.templateDir },
        { name: 'assetDir', path: paths.assetDir }
    ];
    for (const dir of criticalDirs) {
        if (!fs.existsSync(dir.path)) {
            warnings.push(`路径 ${dir.name} 不存在: ${dir.path}`);
        }
    }
