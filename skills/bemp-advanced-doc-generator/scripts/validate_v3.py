...
for i, table in enumerate(tables):
    if len(table.rows) > 0 and len(table.rows[0].cells) >= 3:
        ...
        if '接口路径' in first_row[0] or '接口' in str(first_row):
            ...
            if len(table.rows) >= 8:  # 1表头 + 7接口
                print(f"  - ✓ 包含7个接口定义")
            else:
                print(f"  - ✗ 接口数量不足，期望8行（1表头+7接口），实际{len(table.rows)}行")
            break
...
for i, table in enumerate(tables):
    if len(table.rows) > 0 and len(table.rows[0].cells) >= 6:
        first_row = [cell.text.strip() for cell in table.rows[0].cells]
        if '数据名称' in str(first_row) or '栏位' in str(first_row):
            field_table_found = True
            print(f"表格{i}: 找到栏位描述表格")
            print(f"  - 行数: {len(table.rows)}")
            if len(table.rows) >= 5:  # 1表头 + 4栏位
                print(f"  - ✓ 包含4个栏位定义")
            else:
                print(f"  - ✗ 栏位数量不足")
            break
