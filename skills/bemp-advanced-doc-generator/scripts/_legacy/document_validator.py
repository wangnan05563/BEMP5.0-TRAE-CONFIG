...
from doc_utils import BLUE, is_blue_paragraph, is_hyperlink_run, has_non_hyperlink_blue
...
def validate_document(template_path, generated_path):
    ...
    for i, p in enumerate(generated.paragraphs):
        text = p.text.strip()
        if text.startswith('<') and ('此处填写' in text or '注意' in text or '例如' in text or '如：' in text or '如:' in text):
            placeholder_count += 1
...
def validate_generated_doc(generated_path):
    ...
    for idx, title in enumerate(h1_order):
        if '数据库设计' in title:
            er_h1_found = True
            if idx > 0 and '系统集成' in h1_order[idx - 1]:
                er_after_system_integration = True
            if idx < len(h1_order) - 1 and '附录' in h1_order[idx + 1]:
    ...
    if not er_h1_found:
        result['warnings'].append('未找到"数据库设计"H1章节')
    elif not er_after_system_integration or not er_before_appendix:
        result['warnings'].append('ER章节位置不正确，应在"系统集成"之后、"附录"之前')

    # 5. 占位符检查
    placeholder_count = 0
    for p in generated.paragraphs:
        text = p.text.strip()
        if text.startswith('<') and ('此处填写' in text or '注意' in text or '例如' in text):
            placeholder_count += 1
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Usage: python document-validator.py <template.docx> <generated.docx>', file=sys.stderr)
        sys.exit(1)
    result = validate_document(sys.argv[1], sys.argv[2])
