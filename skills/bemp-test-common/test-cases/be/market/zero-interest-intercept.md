...
| 修订记录 | v1.1（2026-09-01）评审回退局部修复：M-1 新增 TC-015 回购式转贴现（BT02）场景；M-2 TC-001 补充 §7 后端留痕日志断言并标注需求-实现偏差；M-3 贴现表名修正 TE_DISC_BILL→TE_CE_DISC_BILL（含批次表 TE_CE_DISC_BATCH 锚点）；轻微项 L-1/L-2/L-3/L-4/L-5/L-6 |
...
| 代码锚点 | HnnxMarketValidateUtil.java L44-67（validateZeroInterestForRebuy，firstPayInterest，compareTo(BigDecimal.ZERO)==0）；HnnxRebuyApplyServiceImpl.java L100（commitApply）/L210-213（校验调用）；submitCheckMixin.js L101-123（validateZeroInterestBills）；quoteRebuyChange.vue L575-576（interestField=firstPayInterest） |
...
**预期结果**：
1. 弹出提示弹窗，文案精确为"票号{D1票据号}的应付利息为0"（锚点：zh-CN.js L48 warnTip="票号{0}的应付利息为0" + submitCheckMixin.js L115-116 replace("{0}", billNo)）
2. 弹窗仅有"确定"按钮（btnType: inform 告知态，锚点：submitCheckMixin.js L119）
...
6. 【§7 后端留痕断言-限后端路径触发】仅当请求到达后端触发校验时（本用例主流程为前端拦截不发请求、后端日志不产生；本断言在后端兜底场景执行，如 TC-003 步骤 4 直调 commitApply），服务端输出 WARN 留痕日志，内容含拦截动作与票据号："买入端票据[{票据号}]的首期应付利息为0，将拦截整批次提交"（锚点：HnnxMarketValidateUtil.java L63，logger.warn 单参数 billNo，票号经 CdRangeUtil 拼接子票区间）。⚠ 需求-实现偏差待确认：PRD §7 要求记录利息值+操作人，现实现仅记录票据号+时间戳（不含利息值/操作人），本断言按现实现口径（票据号+拦截动作）执行，偏差已列入遗留问题走产品确认/缺陷分派
...
| 代码锚点 | HnnxMarketValidateUtil.java L77-100（validateZeroInterestForSale，SaleBillDto.firstPayInterest）；HnnxBankSaleApplyServiceImpl.java L391（commitApply）/L465（负数校验）/L467-469（零值校验调用）；quoteSaleChange.vue L597-598（interestField=firstPayInterest）、L36（:loading="preSubmitChecking"） |
...
3. 卖出端校验字段为 firstPayInterest 而非 interest（锚点：HnnxMarketValidateUtil.java L88 billDto.getFirstPayInterest()；PRD v3补充2 已修正字段名），断言以 firstPayInterest 命中拦截为准
...
| 代码锚点 | submitCheckMixin.js L113-116（zeroInterestBillNos.map(...).join("；")，一次弹窗全列）；HnnxMarketValidateUtil.java L142-152（throwZeroInterestException，String.join(",", billNos) 一次抛出） |
...
2. 点击"提交申请"
3. 检查弹窗文案内容与弹窗数量
4. 【后端兜底补充验证】通过接口工具直接调用 commitApply（携带同批次报文，绕过前端），观察响应错误信息与服务端 WARN 留痕日志；【直调替代路径】若无接口直调工具，可在开关显式关闭态（TC-007 配置 0）下通过页面提交使请求到达后端，替代本步骤完成后端兜底与留痕日志验证（开关关闭态下后端零值校验整体跳过：HnnxRebuyApplyServiceImpl.java L210 if 不成立，validateZeroInterestForRebuy 不执行、HnnxMarketValidateUtil.java L63 留痕日志不产生，本替代路径仅验证开关关闭放行；后端兜底拦截与留痕日志断言须在开关开启且请求到达后端时执行，仅能通过接口直调主路径完成）

**预期结果**：
1. 前端弹窗文案为"票号{D1a票号}的应付利息为0；票号{D1b票号}的应付利息为0"（多票号以中文分号"；"连接，锚点：submitCheckMixin.js L116 join("；")），且仅弹 1 次窗完整列出全部零息票号，无需多次提交逐个暴露
2. 弹窗不包含正常票 D2 的票号
3. 后端兜底验证（补充项）：commitApply 响应错误信息为"票号{D1a票号},{D1b票号}的应付利息为0"（后端多票号以英文逗号","拼接，锚点：HnnxMarketValidateUtil.java L145 String.join(",") + i18n hnnx_mt_zh_CN.properties L8），错误码 HNNX0BE320008；服务端输出 WARN 留痕日志（TC-001 预期 6 的日志断言在此后端路径执行，锚点：HnnxMarketValidateUtil.java L63）
...
| 代码锚点 | HnnxRebuyApplyServiceImpl.java L210-213（整批校验入口）；HnnxMarketValidateUtil.java L49-65（逐张遍历整批，任意一张命中即收集并整批拦截） |
...
| 代码锚点 | submitCheckMixin.js L117-119（$msgTip.warn，btnType:"inform" 告知态仅确定按钮，不传 duration/autoHide 防止一闪而过） |
...
| 代码锚点 | HnnxMarketValidateUtil.java L164（getParamValue 默认值 "1"）/L166（仅显式 0/N/否 关闭）/L167-172（异常 fail-closed 返回 true）；submitCheckMixin.js L83-88（空值/未配置按开启处理） |
...
**预期结果**：
1. 参数未配置时拦截生效：弹出"票号{D1票据号}的应付利息为0"，提交被阻断（前端 L83-88 空值收敛到校验分支；后端 L164 默认值 "1"）
2. 与 TC-001 行为一致，证明默认态无需任何配置即拦截
3. 【代码审查辅助断言】后端参数服务异常时（getParamValue 抛异常）按开启处理：HnnxMarketValidateUtil.java L167-172 catch 返回 true（fail-closed），日志记录"零利息拦截开关读取异常"
...
| 代码锚点 | HnnxMarketValidateUtil.java L165-166（显式 "0"/"N"(忽略大小写)/"否" 关闭）；submitCheckMixin.js L84-88（flag==="0" || 大写"N" || "否" → resolve(true) 跳过零值校验） |
...
2. 后端 isZeroInterestIntercept 读取到 0 跳过零值校验（锚点 L166），commitApply 返回成功，批次提交成功
3. 开关为 N 与 否 时行为与 0 完全一致（前端大小写不敏感比较 flag.toUpperCase()==="N"，锚点 L85；后端 "N".equalsIgnoreCase(flag)，锚点 L166）
...
| 代码锚点 | 全库 Grep 锚定：isZeroInterestIntercept/validateZeroInterest* 调用仅存在于 4 个提交方法（HnnxRebuyApplyServiceImpl L210、HnnxBankSaleApplyServiceImpl L467、HnnxBankRediscountSaleApplyServiceImpl L221、HnnxBankDiscBillServiceImpl L343），暂存/草稿链路无该校验调用；前端校验链仅绑定提交按钮 runSubmitCheck（quoteRebuyChange.vue L1052-1053 等 4 处） |
...
| 代码锚点 | HnnxBankRediscountSaleApplyServiceImpl.java L215（commitRediscApply 重写父类）/L221-223（isZeroInterestIntercept + validateZeroInterestForRedisc）；HnnxMarketValidateUtil.java L110-133（RediscSaleBillDto.firstPayInterest）；redSaleApplChange.vue L679-680（interestField=firstPayInterest） |
...
| 代码锚点 | submitCheckMixin.js L27（preSubmitChecking 锁定义）/L39-43（执行期重复调用直接 return）/L46/L50（链完成解锁）；quoteRebuyChange.vue L38（:loading="preSubmitChecking"） |
...
| 代码锚点 | HnnxRebuyApplyServiceImpl.java L209-213（commitApply 统一入口：L209 注释"9-0、校验首期应付利息不能为0"，L210 isZeroInterestIntercept + L211-212 throwZeroInterestException，入口内无 busiType 分支，BT01/BT02/BT03 复用同一校验链，PRD §5.2）；quoteManager.vue L91（BT02=质押式回购菜单入口）；quoteRebuyChange.vue L701（bt==='BT02' 复用同一报价编辑页）；HnnxMarketValidateUtil.java L55/L57（firstPayInterest 判零字段） |
...
**预期结果**：
1. 弹出提示弹窗，文案精确为"票号{D1e票据号}的应付利息为0"（与 TC-001 共用同一校验链：前端 submitCheckMixin.js validateZeroInterestBills + 后端 HnnxRebuyApplyServiceImpl L209-213 统一入口，BT02 不分叉，PRD §5.2 BT01/BT02/BT03 复用 commitApply 入口）
...
4. 回购式计息路径（首期+到期两期利息）下，仅首期应付利息为 0 即触发拦截（校验字段=firstPayInterest，到期利息不参与本需求判零，锚点：HnnxMarketValidateUtil.java L55/L57），证明回购式场景无校验链旁路
...
| 代码锚点 | submitCheckMixin.js L108-109（注释明确 Number("")===0 与 Number(" ")===0 陷阱，用 trim 判断排除空串与纯空白串；空利息按 PRD 走现有流程不拦截）；L109 条件表达式 |
...
**测试步骤**：
1. Read submitCheckMixin.js L101-112（validateZeroInterestBills 循环体）
...
**预期结果**：
1. 条件表达式包含 trim 空串排除分支（锚点 L109 逐字一致）
2. 逻辑推演结果：空串""→trim 后为""，条件不成立，不进入 zeroInterestBillNos（不误拦）；空白串"  "→同上；null/undefined→前两个分支排除；"0.00"→Number("0.00")===0 成立，命中拦截
3. 运行时补充（若执行）：空利息票据（空串落库为 NULL 与前端取值为空两种口径）提交均不弹零利息弹窗，走现有利息未计算校验流程（后端 HnnxMarketValidateUtil.java L56 null 视为未计算不在此校验，由 DiscConstants 计息标志等现有校验处理）
...
| 代码锚点 | HnnxRebuyApplyServiceImpl.java L207-208（注释"9、校验结算金额和利息不能为负数"+ RebuyValidateUtil.validateSettleAmtAndInterest）→ L209-213（注释"9-0、校验首期应付利息不能为0…先负数后零值"+ 零值校验）；HnnxBankSaleApplyServiceImpl.java L464-465（validateSettleAmtAndInterest）→ L466-469（零值校验，注释"9-1-1…先负数后零值"） |
...
**测试步骤**：
1. Read HnnxRebuyApplyServiceImpl.java L205-215，确认 commitApply 内负数校验（L208）在零值校验（L210-213）之前
2. Read HnnxBankSaleApplyServiceImpl.java L463-470，确认同构顺序（L465 负数 → L467-469 零值）
...
4. 贴现链路同构佐证：HnnxBankDiscBillServiceImpl.java L367-369（计息标志校验先行首张即抛）→ L370-377（零值循环收集）→ L447（汇总抛出），注释 L445 明确"先到先拦的优先级不变"
...
- 预期结果确定性：通过（文案断言逐字锚定 zh-CN.js L48 / hnnx_mt_zh_CN.properties L8；弹窗按钮态锚定 mixin L117-119；接口路径断言取自 PRD §5.2 与代码；"利率 0 可录入"与"批次提交后状态"两处标测试假设/以实际流转为准；§7 日志断言按现实现口径执行并显式标注需求-实现偏差待确认，未按任一口径默默写死）
...
- 自校验结论：可提交复核（v1.1 评审回退修复完成，待 test-lead-reviewer 复核 M-1/M-2/M-3 三项修复点）
