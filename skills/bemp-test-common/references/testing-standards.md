...
| P0测试用例基准 | `test-cases/` 目录下各子系统子目录 | 已设计的P0用例(约207条)，作为新用例编写的参考基准 |
...
| 优先级 | 定义 | 适用场景 |
|--------|------|----------|
| P0 | 冒烟测试 | 核心资金流程、阻塞性问题、数据安全 |
| P1 | 核心功能 | 主要业务流程、关键交互 |
| P2 | 一般功能 | 辅助功能、非关键交互 |
| P3 | 边界场景 | 管理类功能、异常输入、极端条件 |

## BEMP 特有测试关注点

### 1. 个性化路径验证

所有涉及个性化开发的页面，必须验证 API 请求路径是否使用当前银行的个性化前缀（由 `config/generator-config.json` 中 `banks.{bank_id}.url_prefix` 配置）：

```python
from scripts.health_check import load_config, get_bank_config

config = load_config('../config/generator-config.json')
...
> 错误过滤规则可在 `config/generator-config.json` 中扩展配置
...
通过 `config/generator-config.json` 中的 `active_bank` 字段或命令行 `--bank` 参数切换银行环境：
...
新增银行环境时，只需在 `config/generator-config.json` 的 `banks` 节点下添加对应配置即可。
...
## 实测经验教训（2026-05-17 承兑行额度管理）

### 1. 提示文本断言应使用模糊匹配

需求文档中提示文本可能与实际系统存在细微差异（如"请选中"vs"请选择"），测试断言应使用模糊匹配而非精确匹配：

```python
# 不推荐：精确匹配
assert "请选中一条数据" in page.text_content(".h-message")

# 推荐：模糊匹配
assert "选择一条数据" in page.text_content(".h-message")
```

### 2. 导出功能测试需先验证模板配置

BEMP的清单导出功能依赖数据库中的导出模板配置，测试前应先确认模板已配置：

```sql
SELECT * FROM TM_EXPORT_TEMPLATE WHERE TEMPLATE_NAME = '{导出模板名}';
```

若模板不存在，导出测试应标记为BLOCKED而非FAIL。
...
注意：状态码不是连续的（1→5而非1→2→3），测试断言时需使用实际状态码。
...
路由注册在 `hnnxbankIndex.js` 中，导航时需先点击子系统选项卡触发路由注册。

### 5. 弹窗嵌套层级处理

批复明细功能在额度申请批次弹窗内再打开明细弹窗，形成两层弹窗嵌套。测试时需注意：
- 使用 `page.locator(".h-msg-box:visible")` 的 `.last` 获取最内层弹窗
- 关闭弹窗时从内到外依次关闭
- `dismiss_all_modals()` 可用于清理残留弹窗层