...
- **进程预检机制**：启动前先检测指定端口及服务进程是否已运行，若已运行则输出详细信息并终止启动流程，从根本上杜绝重复启动
- **独立终端运行**：每个服务在独立的 IDE 终端中以前台方式运行,日志直接可见
- **跨平台端口检测**：优先使用 `Get-NetTCPConnection`，自动回退到 `netstat`，兼容 Windows / Linux / macOS
- **进程级精准验证**：不仅检测端口占用，还验证占用进程名是否匹配目标服务（如前端仅识别 `node.exe`，后端仅识别 `java.exe`），避免误判
- **状态检测**：自动检测各服务端口占用情况
