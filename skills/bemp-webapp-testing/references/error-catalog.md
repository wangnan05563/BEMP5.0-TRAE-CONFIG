...
| ⏸ 项交付后无法恢复执行 | 22. ⏸ 补验闭环缺失 |
| 并行测试中列表行数/内容与预期不符 | 23. 并行任务时序快照冲突 |
...
| ⏸项交付后无法恢复执行 | 补验闭环缺失（回归条件未登记） | 22 |
| 并行测试列表断言时对时错 | 并行任务时序快照冲突 | 23 |
...
**现象**：用例/检查项被标 ⏸（BLOCKED）交付后，无人知道何时、以何条件可以恢复执行，暂缓项永久滞留，回归时被遗漏。

**根因**：只标了 ⏸ 未登记回归条件（何种数据/配置/外部依赖就绪后可执行），且缺少"条件触发 → 定向补验 → 结果回写原用例"的闭环动作。

**处置方案**：按 P13 补验闭环执行——⏸ 项登记回归条件（数据/配置/外部依赖三类，写明可核验的就绪标志）→ 条件触发后二轮定向补验 → 补验结果（结论与证据）回写原用例。与 P6 外部依赖标注协议衔接：P6 管标注时写条件，P13 管条件触发后补验并回写。

**预防措施**：凡标 ⏸ 必须同时登记回归条件，禁止空标注；交付文档单列"⏸ 项清单及回归条件"章节；范围裁剪项（明确不做）标"范围外"而非 ⏸，不进入补验闭环。

## 23. 并行任务时序快照冲突

### 23.1 并行测试中列表行数/内容断言与预期不符

**现象**：并行回归/E2E 场景中，页面列表行数、内容类断言结果"时对时错"，或与他人执行结果互相矛盾；代码与数据排查均无缺陷。

**根因**：多智能体并行操作同一 DB，断言时刻他人事务窗口未闭合（数据未落库/未清理），断言读到的是中间时序快照而非终态（P14）。

**处置方案**：①并行任务的数据断言标注"时序快照"，判定缺陷前先确认断言时刻是否有他人事务在途；②任务编排按"谁生产谁清理、错开生命周期"，避免同一数据集被并行读写；③终态结论以清理后复核为准（清理动作全部完成后做终态核对断言）。

**预防措施**：并行前明确数据隔离边界（独立业务键段/独立数据集）；无法隔离时列表行数/内容类断言一律标注时序快照并在报告中注明，终态复核单列一轮。


## 24. 运行进程代码版本失配（旧代码运行时）

### 24.1 已下线接口仍可达 / 新校验文案不生效

**现象**：测试中已删除/下线的接口仍返回业务响应（HTTP 200 + 权限类错误码），或新功能的校验文案/参数行为不出现；用例大面积假失败、假 BLOCKED，排查代码却始终"看起来已修复"。

**根因**：源码已编译、deploy jar 已重打（W9-01 通过），但 Served 进程启动时间早于编译/部署时间——运行进程未重启，加载的仍是旧代码。jar 层新鲜度检查（W9-01）与进程层新鲜度是两道独立检查，缺一道即失守（W12-01）。

**处置方案**：按 P17 三重新鲜度验证——①进程启动时间（WMI/ps 获取）②部署产物 mtime ③最新源码修改时间（git log -1），三者任一倒挂即判旧代码运行时，走 bemp-automation-startserver 门禁通道重启后重检；铁证判别=对已删除接口发请求，旧代码 200（路由仍匹配）、新代码 404/路径不存在。

**预防措施**：本轮存在后端代码变更时，测试执行前强制执行 runtime-code-version-check（已配置化：config/data-readiness-check.json）；启动服务智能体完成重启后必须报告"新启动时间 vs 本次编译时间"供下游核验；"锁版本"专项回归场景显式声明跳过并在报告注明。

## 25. 环境配置数据未就绪（菜单/任务/参数缺登记）

### 25.1 执行中集中发现菜单未注册/定时任务未登记/业务参数未配置

**现象**：测试执行中（而非执行前）发现页面菜单缺失、抽数任务无法触发、参数读取失败，大量用例被迫标 BLOCKED，紧急补数据打乱执行节奏。

**根因**：数据就绪预检只覆盖业务数据（表/记录/文件），未覆盖"环境配置数据"类——菜单权限注册、定时任务登记（TT_TASK 类）、业务参数配置（W12-02）。

**处置方案**：按 P18 执行环境配置数据预检（已配置化：config/data-readiness-check.json env-config-data-precheck）——预检 SQL 声明在配置中（表名/字段/编码值经解析链获取），逐项回读比对期望；未就绪项判断属交付 SQL 未执行（补执行+确认脚本入增量目录）还是外部依赖（标 BLOCKED+回归条件）。

**预防措施**：新环境首次执行、或本轮涉及菜单/参数/任务类变更时，预检强制开启；回读通道必须与写库通道同 schema（先过 db-connection-schema-check，W12-03）。
