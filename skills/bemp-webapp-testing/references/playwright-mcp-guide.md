...
| 主题 | 跳转章节 |
|:---|:---|
| MCP 工具清单（导航/交互/内容/网络/高级） | 1. Playwright MCP 工具概述 |
| 登录流程与验证 | 2. BEMP 登录工作流、11. 登录深度解析（加密/字段映射/锁定） |
| Vue 懒加载路由菜单导航与回退 | 3. Vue 动态路由导航工作流 |
| datagrid/弹窗/下拉/日期/金额输入 MCP 操作 | 4. HUI 组件交互工作流 |
| 状态机流转验证 | 5. 状态流转验证工作流 |
| API 监听与个性化前缀校验 | 6. API 请求监控工作流 |
| 测试数据创建/清理/账号 | 7. 测试数据管理策略 |
| 断言写法（文本/元素/行数/状态/API） | 8. 断言策略 |
| 端到端完整示例 | 9. 完整测试流程示例 |
| 批量导入/文件上传（fileChooser） | 10. 批量导入与文件上传工作流 |
| 数据隔离双账号对比 | 12. 数据隔离验证工作流 |
| 弹窗重叠 BUG 处理 | 13. 弹窗重叠BUG处理 |
| MCP vs 完整脚本选型 | 14. 能力对比 |
