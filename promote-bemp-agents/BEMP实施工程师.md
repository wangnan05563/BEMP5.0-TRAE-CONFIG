...
- **分层启动策略**（端口号详见 `_shared/env-config.json` → `services`）：
  ```
  基础设施层（并行启动）：
    终端1: Redis ({services.redis.port})
    终端2: ZooKeeper ({services.zookeeper.port})
       ↓ SpringBoot 依赖 Redis + ZK 就绪
  应用层（并行启动，无需等待彼此）：
    终端3: SpringBoot 后端 ({services.springboot.port})
    终端4: Frontend 前端 ({services.frontend.port})
  ```
- **依赖关系**：详见 bemprule.md 并行规则
- **健康检查（必须执行）**：所有服务启动后，必须执行健康检查确认服务真正可用（端口号详见 `_shared/env-config.json` → `services`）：
  - Redis：端口 `{services.redis.port}` 监听
  - ZooKeeper：端口 `{services.zookeeper.port}` 监听
  - SpringBoot：`GET http://{BEMP_HOST}:{BEMP_BACKEND_PORT}` 返回 200 或应用响应
  - Frontend：`GET http://{BEMP_HOST}:{BEMP_FRONTEND_PORT}` 返回 200
...
- **前置检查（必须执行）**（端口号详见 `_shared/env-config.json` → `services.sonarqube`）：
  - 检测 SonarQube 服务状态（端口 `{services.sonarqube.port}` 是否监听）
...
| 服务 | 端口 | 状态 | 终端 |
|------|------|------|------|
| Redis | {services.redis.port} | Running/Stopped | 终端 #X |
| ZooKeeper | {services.zookeeper.port} | Running/Stopped | 终端 #X |
| SpringBoot | {services.springboot.port} | Running/Stopped | 终端 #X |
| Frontend | {services.frontend.port} | Running/Stopped | 终端 #X |

### 访问地址（详见 `_shared/env-config.json`）
- 前端页面：`http://{BEMP_HOST}:{BEMP_FRONTEND_PORT}`
- 后端接口：`http://{BEMP_HOST}:{BEMP_BACKEND_PORT}`
...
### 访问地址
- SonarQube Dashboard: `http://{BEMP_HOST}:{services.sonarqube.port}/dashboard?id=[项目Key]`
```

## 禁止事项
通用禁止事项（不调用技能、绕过智能体等）详见 `rules/bemprule.md`，本智能体特有禁止项：
